...
class TransformerDSSM:
        ...
        with tf.variable_scope('presentation_layer', reuse=tf.AUTO_REUSE):
            with tf.name_scope('structure_presentation_layer'):
                ...
                if self.is_train and not self.is_extract:
                    output, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_drop_cell, cell_bw=bw_drop_cell, inputs=inputs, sequence_length=inputs_actual_length, dtype=tf.float32)
                else:
                    output, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell, cell_bw=bw_cell, inputs=inputs, sequence_length=inputs_actual_length, dtype=tf.float32)
            ...
            with tf.name_scope('global_attention_layer'):
                w_omega = tf.get_variable(name='w_omega', shape=[self.hidden_num * 2, self.attention_num], initializer=tf.random_normal_initializer())
            ...
            if not self.is_extract:
                ...
                if not self.is_train:
                    ...
                    with tf.name_scope('Accuracy'):
                        ...
                        self.accuracy = tf.reduce_sum(tf.cast(tf.equal(output_train, tf.zeros_like(output_train)), dtype=tf.float32)) / self.batch_size
                    ...
                    for i in range(gpu_num):
                        with tf.device("/gpu:%d" % i):
                            with tf.name_scope("tower_%d" % i):
                                ...
                                q_inputs_actual_length = self.q_inputs_actual_length[i * self.batch_size:(i + 1) * self.batch_size]
                                t_inputs_actual_length = self.t_inputs_actual_length[i * self.batch_size:(i + 1) * self.batch_size]
                                ...
                                with tf.name_scope('Accuracy'):
                                    output_train = tf.argmax(prob, axis=1)
                                    cur_accuracy = tf.reduce_sum(tf.cast(tf.equal(output_train, tf.zeros_like(output_train)), dtype=tf.float32)) / self.batch_size
    ...
    def inference(self, top_k):
        feed_dict = {self.q_inputs: self.q_set, self.q_inputs_actual_length: self.q_actual_length, self.t_final_state: self.t_set, self.top_k_answer: top_k}
...
class FeedFowardNetwork(tf.layers.Layer):
    def __init__(self, hidden_size, keep_prob):
        ...
        self.hidden_size = hidden_size
        self.keep_prob = keep_prob
        self.filter_dense_layer = tf.layers.Dense(self.hidden_size, use_bias=True, activation=tf.nn.swish, name="filter_layer")
